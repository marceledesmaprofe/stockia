        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-2xl font-semibold text-gray-800">Purchase #{{ $purchase->id }} Details</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="space-y-4">
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Purchase ID</h3>
                            <p class="mt-1 text-lg font-medium text-gray-900">#{{ $purchase->id }}</p>
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Date</h3>
                            <p class="mt-1 text-gray-900">{{ $purchase->purchase_date->format('M d, Y') }}</p>
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Supplier</h3>
                            <p class="mt-1 text-gray-900">{{ $purchase->supplier ? $purchase->supplier->full_name : 'Walk-in Supplier' }}</p>
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Payment Method</h3>
                            <p class="mt-1 text-gray-900">{{ $purchase->payment_method ?? '-' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-gray-900">{{ $purchase->user->name }}</p>
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Created At</h3>
                            <p class="mt-1 text-gray-900">{{ $purchase->created_at->format('M d, Y H:i') }}</p>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tfoot class="bg-gray-50">
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">${{ number_format($purchase->total, 2) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg mb-6">
                    ...
                    <p class="text-sm text-gray-600">
                        This purchase automatically generated stock movements (ENTRADA) for each product.
                        If you need to correct an error, annul the purchase to create reverse movements (SALIDA).
                    </p>
                </div>

                <div class="flex space-x-3">
                    @if($purchase->status === 'PENDIENTE')
                        <form action="{{ route('purchases.mark-as-paid', $purchase->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to mark this purchase as paid?');">
                            @csrf
                            @method('PATCH')
                    ...
                    @if($purchase->status !== 'ANULADA')
                        <form action="{{ route('purchases.annul', $purchase->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to annul this purchase? This will remove the stock for all products.');">
                            @csrf
                            @method('PATCH')
